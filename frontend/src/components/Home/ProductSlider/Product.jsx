...
const Product = (props) => {
    ...
    return (
        <div className="flex flex-col items-center gap-2 px-2 py-6 relative shadow-lg">
            {/* <!-- image & product title --> */}
            <Link to={`/product/${_id}`} className="flex flex-col items-center text-center group w-full">
                ...
                <h2 className="text-xl mt-4 font-medium group-hover:text-primary-green">{name.length > 50 ? `${name.substring(0, 50)}...` : name}</h2>
            </Link>
            {/* <!-- image & product title --> */}
